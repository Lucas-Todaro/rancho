...
import type { AnyRecord, ModuleConfig } from "@/lib/types";
...
function calcStat(rows: AnyRecord[], stat: NonNullable<ModuleConfig["quickStats"]>[number]) {
  ...
  if (stat.mode === "critical") return rows.filter((row) => Number(row.quantity || 0) <= Number(row.min_quantity || 0)).length;
  if (stat.mode === "moneyIn") return formatCurrency(rows.filter((row) => row.type === "receita").reduce((sum, row) => sum + Number(row[stat.field] || 0), 0));
  if (stat.mode === "moneyOut") return formatCurrency(rows.filter((row) => row.type === "despesa").reduce((sum, row) => sum + Number(row[stat.field] || 0), 0));
  ...
  if (["salary", "amount", "net_salary"].includes(stat.field)) return formatCurrency(sum);
...
export function ModuleScreen({ config }: { config: ModuleConfig }) {
  ...
  async function load() {
    ...
    setError("");
    try {
      const data = await listRecords(config.tableName, config.orderBy);
      setRows(data);
    } catch (err) {
  ...
  async function submit(values: AnyRecord) {
    ...
    try {
      if (editing?.id) {
        ...
        await createRecord(config.tableName, values);
      }
      await load();
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200">
            <Icon className="h-4 w-4" /> {config.title}
          </div>
          <h1 className="text-3xl font-black tracking-tight md:text-5xl">{config.title}</h1>
      ...
      {error ? <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-700 dark:border-red-900 dark:bg-red-950 dark:text-red-200">{error}</div> : null}
      ...
      <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
        <ModuleForm config={config} editing={editing} onSubmit={submit} onCancel={() => setEditing(null)} busy={busy} />
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-black">Registros</h2>
            <div className="flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-600 dark:bg-slate-800 dark:text-slate-300">
              <Plus className="h-4 w-4" /> {loading ? "Carregando..." : `${filteredRows.length} itens`}
            </div>
          </div>
          <DataTable rows={filteredRows} fields={config.fields} search={search} setSearch={setSearch} onDelete={remove} onEdit={setEditing} onExport={() => exportCsv(config.key, filteredRows, config.fields)} />
        </div>
      </div>
    </div>
  );

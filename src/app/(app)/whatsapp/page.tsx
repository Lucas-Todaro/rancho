...
export default function WhatsAppPage() {
  ...
  async function sendTest() {
    ...
    setStatus(data.ok ? "Menu enviado. Confira o WhatsApp." : "Nao foi possivel enviar agora. Confira se o WhatsApp esta ativo.");
  }

  return (
    <div className="animate-fade-in space-y-6">
      <section className="overflow-hidden rounded-lg bg-emerald-950 p-6 text-white shadow-soft md:p-8">
        <Badge tone="success">Atendimento rapido</Badge>
        ...
        <div className="glass rounded-lg p-5 shadow-soft md:p-6">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Menos digitação e menos perda de informacao.</p>
            </div>
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            {[
              "Recebe menus com botoes simples.",
              "Guia o preenchimento passo a passo.",
              "Evita esquecer litros, valores e observacoes.",
              "Mantem os registros aparecendo no painel."
            ].map((item) => (
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {[
          { icon: Smartphone, title: "Menu inicial", text: "Opcoes claras para comecar um registro pelo telefone." },
          { icon: Bot, title: "Fluxo guiado", text: "O atendimento pergunta uma coisa por vez ate completar o registro." },
          { icon: CheckCircle2, title: "Painel atualizado", text: "As informacoes aparecem nas areas certas do sistema." }
        ].map((item) => {
      ...
      <div className="glass rounded-lg p-5 shadow-soft md:p-6">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <div className="rounded-lg bg-emerald-50 p-4 dark:bg-emerald-950/30"><strong>Ordenha</strong><p className="mt-2 text-sm">Brinco do animal, litros e observacoes.</p></div>
          <div className="rounded-lg bg-blue-50 p-4 dark:bg-blue-950/30"><strong>Animal</strong><p className="mt-2 text-sm">Cadastro basico com brinco, categoria e raca.</p></div>
          <div className="rounded-lg bg-amber-50 p-4 dark:bg-amber-950/30"><strong>Financeiro</strong><p className="mt-2 text-sm">Entrada ou saida com valor e descricao.</p></div>
        </div>
      </div>
    </div>
  );

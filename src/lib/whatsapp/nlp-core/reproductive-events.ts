...
export function detectReproductiveEventKind(text: string): ReproductiveEventKind | undefined {
  ...
  if (/\b(?:pre\s*parto|pre-parto|preparto|perto de parir|quase parindo|para parir|final da gestacao|fim da gestacao)\b/.test(normalized)) {
    return "pre_parto";
  }

  if (/\b(?:pariu|parto|deu cria|teve cria|nascimento|nasceu bezerro|nasceu bezerra)\b/.test(normalized)) {
    return "parto";
  }
  ...
  if (!/\b(?:nao esta prenha|nao ficou prenha|prenhez negativa|diagnostico negativo de prenhez)\b/.test(normalized)
    && /\b(?:confirmar prenhez|prenhez positiva|diagnostico positivo|esta gestante|esta prenha|ficou prenha|pegou cria|prenhas?|prenhes|prenhe|prenhez|gestantes?|gestacao)\b/.test(normalized)) {
    return "prenhez";
  }
  ...
  if (/\b(?:cio|aborto)\b/.test(normalized)) return "observacao";

  return undefined;
}

export function hasReproductiveEventCue(text: string) {
  return reproductiveCuePattern.test(normalizeRanchoText(text));
}
